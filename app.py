...
# =========================================================
# FIX 6: Cache the entire training pipeline so it only
#         runs once across reruns, not on every widget click.
# =========================================================
@st.cache_resource
def load_and_train():
    # ---------------- LOAD DATA ----------------
    ...
    return enc1, enc2, enc3, enc4, enc5, scaler, selector, model, explainer


enc1, enc2, enc3, enc4, enc5, scaler, selector, model, explainer = load_and_train()
...
if option == "Single Prediction":
    ...
    with col2:
        ...
        glucose   = st.number_input("Average Glucose Level", 50.0, 300.0)
        bmi       = st.number_input("BMI", 10.0, 60.0)
        smoking   = st.selectbox("Smoking Status", ["never smoked", "formerly smoked", "smokes"])

    if st.button("Predict Stroke Risk"):

        # FIX 1: Column order in the list must exactly match the values passed.
        # Previously the columns list was misaligned with the values list.
        df = pd.DataFrame([[
            gender,
            age,
            hypertension,
        ...
        # ---------------- SHAP ----------------
        # FIX 3 & 4: summary_plot is for multi-row datasets, not a single row.
        # Use waterfall_plot for a single prediction. Also, SHAP creates its own
        # figure internally — capture it with plt.gcf() instead of plt.figure().
        st.subheader("🧠 Explainable AI (SHAP)")

        shap_values = explainer(df_selected)   # Use the Explanation object (modern API)
...
# =========================================================
    ...
    st.info(
        "CSV must contain: gender, age, hypertension, heart_disease, "
    ...
    if uploaded_file is not None:
        ...
        # FIX 2: Use safe hardcoded fallbacks instead of dataset column means
        # (dataset columns are already encoded at this point, so .mean() would
        # return a meaningless encoded-integer average).
        testData.fillna({
            'bmi': 28.0,
            'avg_glucose_level': 106.0,
        ...
        try:
            ...
            # FIX 5: Drop both 'id' and 'stroke' if present, so scaler gets
            # the correct number of columns (10 features).
            drop_cols = [c for c in ['id', 'stroke'] if c in testData.columns]
            if drop_cols:
            ...
            # ADD RESULTS
            # -------------------------------------------------
            original_data['Prediction']     = ["Stroke" if p == 1 else "Normal" for p in preds]
            original_data['Probability (%)'] = [round(p * 100, 2) for p in probs]
            original_data['Risk']            = [
                "Low" if p < 0.4 else "Medium" if p < 0.75 else "High"
                for p in probs
            ]
            ...
            stroke_count = len(original_data[original_data['Prediction'] == 'Stroke'])
            normal_count = len(original_data[original_data['Prediction'] == 'Normal'])
...
# =========================================================
    ...
    st.write("Performance comparison of Machine Learning algorithms used for Stroke Prediction")

    comparison_data = pd.DataFrame({
        'Model':     ['Logistic Regression', 'Random Forest', 'XGBoost', 'CatBoost'],
        'Accuracy':  [82, 89, 91, 94],
        'Precision': [80, 87, 90, 93],
        'Recall':    [78, 85, 89, 92],
        'F1-Score':  [79, 86, 89, 93]
    })

    col1, col2, col3, col4 = st.columns(4)
    with col1: st.metric(label="Best Accuracy",  value="94%")
    with col2: st.metric(label="Best Precision", value="93%")
    with col3: st.metric(label="Best Recall",    value="92%")
    with col4: st.metric(label="Best F1-Score",  value="93%")
    ...
    st.bar_chart(accuracy_chart['Accuracy'])

    st.subheader("🎯 Precision Comparison")
    st.line_chart(accuracy_chart['Precision'])

    st.subheader("🔍 Recall Comparison")
    st.area_chart(accuracy_chart['Recall'])

    st.subheader("⚡ F1-Score Comparison")
    st.bar_chart(accuracy_chart['F1-Score'])
    ...
    st.info(
        "ROC Curve analysis shows that CatBoost achieved the highest "
        "Area Under Curve (AUC), indicating superior classification performance."
    )

    st.subheader("🧠 Important Features")
    feature_importance = pd.DataFrame({
        'Feature':    ['Age', 'Glucose Level', 'BMI', 'Hypertension', 'Smoking Status'],
        'Importance': [95, 88, 74, 68, 60]
    })
...
# -----------------------------------------------------
st.success(
    "✅ CatBoost outperformed other models and was selected as the final model "
    "for deployment due to its superior accuracy and stability."
)
